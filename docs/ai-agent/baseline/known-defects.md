...
| 47 | `.github/dependabot.yml` | Docker-ecosystem для `/` и `/deploy/docker` указывал на каталоги без Dockerfile → два падающих Dependabot-рана каждую неделю (подтверждено 2026-09-13 и 2026-09-15); digest-пины compose (`mysql:8.0`) не покрывались | исправлено (Iteration 67 follow-up: мёртвые entries удалены, compose-пины — вручную) |
