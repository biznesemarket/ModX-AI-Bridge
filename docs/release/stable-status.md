...
Status: **STABLE** — current release `0.11.0` (previous: `0.10.0`, `0.9.3`, `0.9.2`, `0.9.1`, `0.9.0`, `0.8.0`, `0.7.1`, `0.7.0`, `0.6.0`, `0.5.0`, `0.4.0`, `0.3.0`, `0.2.0`, `0.1.3`, `0.1.2`, `0.1.1`, `0.1.0`).

## 0.11.0 (current)

`AIBRIDGE_RUNTIME=1 bash scripts/certification/stable-gate.sh` completed green on GitHub Actions
`ubuntu-latest`, commit `452f1db`, run `34973881413`:
...
composer test-modx                                   == MODX INTEGRATION: PASS ==
  ...
  full PHPUnit                                       OK (190 tests, 1219 assertions)
package reproducibility                              PACKAGE REPRODUCIBILITY: PASS (sha256 1f23f9fd…)
STABLE certification gates passed.
```

- Release artifact: `aibridge-0.11.0.transport.zip`, SHA-256
  `1f23f9fd62df99d26a0b923301dac17010aa25ee6b9f240774dda4096d9e0bb2` (148228 bytes); local build == CI
  artifact. SDK `User-Agent` moves to `0.11` and SDK/package `clientInfo` to `0.11.0` (minor release); all
  version identities, including README, were committed before the gate.
- Push-CI on `452f1db`: `Quality Gates` `34973874041`, `MODX Integration` `34973873975` — success.
...
composer test-modx                                   == MODX INTEGRATION: PASS ==
  step 10b TypeScript SDK live HTTP E2E              TYPESCRIPT SDK LIVE HTTP: PASS
  full PHPUnit                                       OK (186 tests, 1094 assertions)
package reproducibility                              PACKAGE REPRODUCIBILITY: PASS (sha256 4ca7d0a8…)
./scripts/quality-gate.sh                            PASS
STABLE certification gates passed.
```

- Release artifact: `aibridge-0.10.0.transport.zip`, SHA-256
  `4ca7d0a8f2b7e97066833d6eedafb174c6e5c2772bef9bb8c97543acee4227bf` (144514 bytes); local build == CI
  artifact. SDK `User-Agent` moves to `0.10` and SDK/package `clientInfo` to `0.10.0` (minor release); all
  version identities, including README, were committed before the gate.
- Push-CI on `0423714`: `Quality Gates` `34809039290`, `MODX Integration` `34809039317` — success.
...
- Release artifact: `aibridge-0.9.3.transport.zip`, SHA-256
  `24f50ea9eb512cadee11e22c5112672a8b7b257d63cc85b41cb0fda5ba7f59e9` (148130 bytes); local build == CI
  artifact. README and all version identities were committed before the gate (SDK `clientInfo` bumps to
  `0.9.3`; the SDK `User-Agent` stays `0.9` for patch releases).
- Patch release over `0.9.2`: publish approval bound to profile/operation/resource (cross-profile and replay
  rejections), manager-connector MCP channel propagation, workflow `after_json`/`published` verification fix,
  MCP `resource_publish` schema (`change_id` required), docs corrections and test-fixture cleanup.
- Fix/coverage only: no scope, route, schema/migration change or API contract break.
- See `docs/release/0.9.3.md`.
