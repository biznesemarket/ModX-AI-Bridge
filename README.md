...
Status: **Stable `0.4.0`** (tag `v0.4.0`; certified `aibridge-0.4.0.transport.zip`)
...
template variables to the read-back projection (`tvs`) and the `resource.read` capability; `0.4.0` is the
current Stable minor release and adds the filtered, paginated resource list (`GET /api/ai/v2/resources` and
the `resource_list` MCP tool). Evidence:
`docs/release/0.4.0.md`, `docs/release/stable-status.md`, `docs/testing/STATUS.md`.
