...
final class TokenManager
{
    ...
    public function issue(string $name,array $scopes,?int $ttlDays=null): array
    {
        $plain=bin2hex(random_bytes(32)); $hash=hash('sha256',$plain); $days=$ttlDays??(int)$this->config->get('token_expiry_days',90);
        $row=$this->modx->newObject(\AIBridge\Model\Token::class); $row->fromArray(['name'=>$name,'token_hash'=>$hash,'scopes_json'=>json_encode(array_values(array_unique(array_map('strval',$scopes)))),'status'=>'active','expires_at'=>date('Y-m-d H:i:s',time()+$days*86400),'created_at'=>date('Y-m-d H:i:s')]);
