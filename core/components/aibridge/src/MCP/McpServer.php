...
final class McpServer
{
    ...
    private function register(): void {
        ...
        $this->registry->tool('resource_read','Read a persisted MODX resource by id.',['type'=>'object','required'=>['id'],'properties'=>['id'=>['type'=>'integer','minimum'=>1]]],fn(array $a)=>$this->application->resourceRead((int)($a['id']??0)),'resource.read');
        $resourceListSchema=['type'=>'object','properties'=>['parent'=>['type'=>'integer','minimum'=>0],'template'=>['type'=>'integer','minimum'=>1],'context_key'=>['type'=>'string'],'published'=>['type'=>'boolean'],'tv_name'=>['type'=>'string'],'tv_value'=>['type'=>'string'],'q'=>['type'=>'string'],'limit'=>['type'=>'integer','minimum'=>1,'maximum'=>100],'offset'=>['type'=>'integer','minimum'=>0],'sort'=>['type'=>'string','enum'=>['id','pagetitle','alias','menuindex','editedon','createdon','publishedon']],'dir'=>['type'=>'string','enum'=>['asc','desc']]]];
        $this->registry->tool('resource_list','List persisted MODX resources with filters and pagination.',$resourceListSchema,fn(array $a)=>$this->application->resourceList($a),'resource.list');
        ...
        $this->registry->tool('resource_publish','Publish a MODX resource subject to approval and policy.',['type'=>'object','required'=>['id','idempotency_key','approval_id'],'properties'=>['id'=>['type'=>'integer','minimum'=>1],'idempotency_key'=>['type'=>'string','minLength'=>8],'approval_id'=>['type'=>'string','minLength'=>1],'change_id'=>['type'=>'integer','minimum'=>1]]],fn(array $a)=>$this->application->resourcePublish($a, $this->principalFromArguments($a), ['ip'=>(string)($a['_client_ip']??''),'idempotency_key'=>(string)($a['idempotency_key']??''),'approval_id'=>(string)($a['approval_id']??''),'change_id'=>(int)($a['change_id']??0)]),'resource.publish');
        ...
        $this->registry->resourceTemplate('modx://resource/{id}','Resource','Read a persisted MODX resource by id','application/json',function(array $a){ $id=(int)($a['id']??0); $result=$this->application->resourceRead($id); if(($result['success']??false)!==true){ return ['error'=>['code'=>-32002,'message'=>'Resource not found','data'=>['uri'=>'modx://resource/'.$id,'reason'=>(string)($result['error']['code']??'resource_not_found')]]]; } return ['contents'=>[['uri'=>'modx://resource/'.$id,'mimeType'=>'application/json','text'=>json_encode($result,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)]]]; },'resource.read');
    ...
    private function principalFromArguments(array $arguments): array
    {
        ...
        return ['type'=>(string)($principal['type']??'token'),'id'=>(string)($principal['id']??'mcp'),'scopes'=>array_values(array_map('strval',$principal['scopes']??[])),'profile_id'=>(int)($principal['profile_id']??0)];
    }

    public function capabilities(): array { return ['protocolVersion'=>McpProtocol::PROTOCOL_VERSION,'tools'=>$this->registry->tools(),'resources'=>$this->registry->resources(),'resourceTemplates'=>$this->registry->resourceTemplates(),'prompts'=>$this->registry->prompts()]; }
    public function handle(array $request,array $principal=[]): array {
        ...
        if($method==='resources/templates/list') return $this->protocol->result($id,['resourceTemplates'=>$this->registry->resourceTemplates()]);
    ...
    private function readResource($id,array $params,array $principal): array { $uri=(string)($params['uri']??''); $resolved=$this->registry->resolveResource($uri); if(!$resolved)return $this->protocol->error($id,-32002,'Resource not found',['uri'=>$uri]); $resource=$resolved['resource']; $arguments=$resolved['parameters']; $d=$this->security->decide(['ip'=>(string)($params['_client_ip']??''),'mcp_resource'=>$uri],$principal,$resource['operation']); if(!$d->allowed())return $this->protocol->error($id,-32003,'Security policy denied',$d->toArray()); try { $out=($resource['handler'])($arguments,$principal); if(isset($out['error'])){ $e=$out['error']; return $this->protocol->error($id,(int)($e['code']??-32603),(string)($e['message']??'Resource read failed'),(array)($e['data']??[])); } return $this->protocol->result($id,$out); } catch(\Throwable $e) { return $this->protocol->error($id,-32603,'Resource read failed',['code'=>'resource_read_failed']); } }
