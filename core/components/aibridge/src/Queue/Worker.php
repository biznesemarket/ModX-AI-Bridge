...
final class Worker
{
    ...
    public function runOnce(string $workerId): ?array
    {
        ...
        try {
            ...
            $this->audit?->record('job_failed', ['profile_id' => (int) ($job->raw()['profile_id'] ?? 0), 'job_id' => $job->id(), 'type' => $job->type(), 'status' => $status, 'error' => $error, 'request_id' => (string) ($job->raw()['request_id'] ?? '')]);
