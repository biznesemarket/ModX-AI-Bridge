...
final class ChangeRequestService {
 ...
 public function approve(int $id,int $approvalId,array $principal):array {
  ...
  (new AuditService($this->modx))->record('change_approved',['change_id'=>$id,'approval_id'=>$approvalId,'actor_type'=>$principal['type']??'unknown','actor_id'=>$principal['id']??'']);return $row->toArray();
 }
