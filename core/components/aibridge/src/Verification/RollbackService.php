...
final class RollbackService {
 ...
 public function rollback(int $snapshotId,array $principal,array $request=[]):array {
  ...
  $resource=$this->modx->getObject('modResource',['id'=>$resourceId]);if(!$resource)throw new \RuntimeException('Target resource not found; automatic recreation from snapshot is disabled.');
  ...
  $pdo=$this->modx->getConnection();if(!$pdo)throw new \RuntimeException('MODX database connection unavailable.');
  ...
  (new AuditService($this->modx))->record('resource_rollback',['snapshot_id'=>$snapshotId,'resource_id'=>$resourceId,'actor_type'=>$principal['type']??'unknown','actor_id'=>$principal['id']??'','request_id'=>$request['request_id']??'']);return ['success'=>true,'snapshot_id'=>$snapshotId,'resource_id'=>$resourceId];
 }
 private function applyTvs(\xPDOObject $resource,mixed $tvs):void{if(!is_array($tvs))return;foreach($tvs as $name=>$value){$tv=$this->modx->getObject('modTemplateVar',['name'=>(string)$name]);if($tv)$resource->setTVValue((int)$tv->get('id'),is_scalar($value)?(string)$value:json_encode($value,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));}}
}
